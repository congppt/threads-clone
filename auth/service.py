...
from auth.utils import gen_access_token, gen_refresh_token, get_claims, is_correct_pwd
...
async def authenticate_async(username: str, password: str, db: AsyncSession) -> User:
    ...
    if not user or is_correct_pwd(password, user.hashed_password):
...
async def refresh_access_token(refresh_token: str, db: AsyncSession) -> str:
    ...
    try:
        payload= get_claims(refresh_token)    
...
def create_access_token(user: User) -> str:
    claims = { "id": user.id }
    return gen_access_token(claims)

def create_refresh_token(user: User) -> str:
    claims = { "username": user.username }
    return gen_refresh_token(claims)
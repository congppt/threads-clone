...
@chat_router.post(
    ...
    summary="Create new chat (if not exist for direct type)",
)
...
async def get_chat_messages_async(
    ...
    before_message_id: Annotated[int | None, Query(ge=1)] = None,
    user: User = Depends(get_current_user_async),
    db: AsyncSession = Depends(get_db_async),
):
    messages, has_more = await service.get_chat_messages_async(
        id, limit, before_message_id, user, db
    )
...
async def chat_websocket_endpoint(
    ...
    chat_ids = await service.start_listening_async(ws, user, db)
    rate_limiter = WebSocketRateLimiter(
        times=50, seconds=10, callback=websocket_callback
    )
    try:
        while True:
            try:
                ...
                if not (handler := socket_manager.handlers.get(message_type)):
                    await socket_manager.send_error(
                        f"Type {message_type} was not found"
                    )
                    continue
                await handler(ws=ws, db=db, data=data, user=user, chat_ids=chat_ids)
                ...
                logger.exception(f"Websocket error, detail: {excinfo}")
                await socket_manager.send_error(
                    "Could not validate incoming message", ws
                )

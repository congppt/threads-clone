...
async def new_message_handle_async(data: dict, user: User, chat_ids: list[Chat], ws: WebSocket, db: AsyncSession, **kwargs):
    ...
    new_message = Message(content = message.content, chat_id = message.chat_id, user_id = user.id)
    db.add(new_message)
    await db.flush()
    ...
    await db.commit()
    await db.refresh(chat)
    send_message = SendMessage(message_id=new_message.id, content=new_message.content, chat_id=new_message.chat_id, user_id=new_message.user_id, created_at=new_message.created_at)
    outgoint_message = send_message.model_dump_json()
    await socket_manager.broadcast_to_channel_async(new_message.chat_id, outgoint_message)